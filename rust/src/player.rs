...
use godot::prelude::*;
use godot::engine::{AnimatedSprite3D, CharacterBody3D, ICharacterBody3D, InputEvent, InputEventMouseMotion, MeshInstance3D, RayCast3D, Texture2D, TextureRect, Timer, Tween};
use rand::Rng;
...
impl ICharacterBody3D for Player {
    ...
    fn ready(&mut self) {
        ...
        self.raycast.init(self.base().get_node_as("Head/Camera/RayCast"));
        self.muzzle.init(self.base().get_node_as("Head/Camera/SubViewportContainer/SubViewport/CameraItem/Muzzle"));
        self.container.init(self.base().get_node_as("Head/Camera/SubViewportContainer/SubViewport/CameraItem/Container"));
        self.sound_footsteps.init(self.base().get_node_as("SoundFootsteps"));
        self.blaster_cooldown.init(self.base().get_node_as("Cooldown"));
    ...
    fn physics_process(&mut self, delta: f64) {
        ...
        let mut applied_velocity = self.base().get_velocity().lerp(self.movement_velocity, (delta * 10.0) as f32);
        ...
        camera_rotation.z = lerp_angle(camera_rotation.z as f64, self.input_mouse.x as f64 * 25.0 * delta, delta * 5.0) as f32;
        camera_rotation.x = lerp_angle(camera_rotation.x as f64, self.rotation_target.x as f64, delta * 25.0) as f32;
        self.camera.set_rotation(camera_rotation);

        let mut rotation = self.base().get_rotation();
        rotation.y = lerp_angle(rotation.y as f64, self.rotation_target.y as f64, delta * 25.0) as f32;
        self.base_mut().set_rotation(rotation);

        let position = lerp(Variant::from(self.container.get_position()), Variant::from(self.container_offset - (applied_velocity / 30.0)), Variant::from(delta * 10.0));
        ...
        camera_position.y = lerp(Variant::from(camera_position.y), Variant::from(0.0f32), Variant::from((delta * 5.0) as f32)).to::<f32>();
...
impl Player {
    pub fn handle_controls(&mut self, _delta: f64) {
        ...
        let input_vector = input.get_vector("move_left".into(), "move_right".into(), "move_forward".into(), "move_back".into());

        self.movement_velocity = Vector3::new(input_vector.x, 0.0, input_vector.y).normalized() * self.movement_speed;

        let rotation_input = input.get_vector("camera_right".into(), "camera_left".into(), "camera_down".into(), "camera_up".into());
        self.rotation_target -= Vector3::new(-rotation_input.y, -rotation_input.x, 0.0).limit_length(Some(1.0)) * self.gamepad_sensitivity;
        self.rotation_target.x = self.rotation_target.x.clamp(deg_to_rad(-90.0) as f32, deg_to_rad(90.0) as f32);
    ...
    fn action_shoot(&mut self) {
        ...
        if input.is_action_pressed("shoot".into()) {
            ...
            if let Some(sound_shoot) = self.weapon.as_mut().map(|x| x.get("sound_shoot".into()).to::<GString>()) {
            ...
            self.camera.set_rotation(camera_rotation);
            self.movement_velocity += Vector3::new(0.0, 0.0, self.weapon.as_ref().unwrap().get("knockback".into()).to::<f32>());
            ...
            self.muzzle.set_scale(Vector3::ONE * rand::thread_rng().gen_range(0.40..0.75));
            self.muzzle.set_position(container_position - self.weapon.as_ref().unwrap().get("muzzle_position".into()).to::<Vector3>());

            self.blaster_cooldown.start_ex().time_sec(self.weapon.as_ref().unwrap().get("cooldown".into()).to::<f64>()).done();

            for _ in 0..self.weapon.as_ref().unwrap().get("shot_count".into()).to::<i32>() {
                let spread = self.weapon.as_ref().unwrap().get("spread".into()).to::<f32>();
                ...
                if collider.has_method("damage".into()) {
                    collider.call("damage".into(), &[Variant::from(self.weapon.as_ref().unwrap().get("damage".into()).to::<f32>())]);
                }
                ...
                self.base().get_tree().unwrap().get_root().unwrap().add_child(impact.clone().upcast());

                impact.set_position(self.raycast.get_collision_point() + (self.raycast.get_collision_normal() / 10.0));
                impact.look_at_ex(self.camera.get_global_transform().origin).use_model_front(true).done();
            }
        }
    }
    ...
    fn initiate_change_weapon(&mut self, weapon_index: usize) {
        ...
        tween.tween_property(self.container.clone().upcast(), "position".into(), Variant::from(self.container_offset - Vector3::new(0.0, 1.0, 0.0)), 0.1);
    ...
    fn change_weapon(&mut self) {
        ...
        let mut weapon_model = self.weapon.as_ref().unwrap().get("model".into()).to::<Option<Gd<PackedScene>>>().unwrap().instantiate().unwrap().cast::<Node3D>();
        self.container.add_child(weapon_model.clone().upcast());

        weapon_model.set_position(self.weapon.as_ref().unwrap().get("position".into()).to::<Vector3>());
        weapon_model.set_rotation_degrees(self.weapon.as_ref().unwrap().get("rotation".into()).to::<Vector3>());
    
        for child in weapon_model.find_children_ex("*".into()).type_("MeshInstance3D".into()).done().iter_shared() {
            let mut child = child.cast::<MeshInstance3D>();
            child.set_layer_mask(2);
        }

        self.raycast.set_target_position(Vector3::new(0.0, 0.0, -1.0) * self.weapon.as_ref().unwrap().get("max_distance".into()).to::<f32>());
        self.crosshair.as_mut().unwrap().set_texture(self.weapon.as_ref().unwrap().get("crosshair".into()).to::<Option<Gd<Texture2D>>>().unwrap());
    }
    ...
    fn damage(&mut self, amount: f32) {
        ...
        self.base_mut().emit_signal("health_updated".into(), &[Variant::from(health)]);
